            ...
            <div class="kursus-tab-card kursus-tab-card-strong rounded-3xl p-6"
                ...
                <div class="rounded-3xl bg-white p-4 shadow-sm" style="border: 1px solid var(--detail-card-border);">
                    ...
                    <div class="flex items-center justify-between text-sm text-gray-500">
                        ...
                        <span class="kursus-tab-accent-strong font-semibold" data-asrama-total-display>RM {{ number_format($asramaTotal, 2) }}</span>
                    </div>
                </div>
                ...
                <script>
                  window.updateGuestYuranTotal = function(trigger, type) {
                    ...
                    if (type === 'asrama') {
                      ...
                      if (on) {
                        trigger.className = 'inline-flex items-center gap-3 rounded-full border px-4 py-2 text-sm font-semibold text-white shadow-sm transition focus:outline-none focus:ring-2 dark:border-orange-400 dark:bg-orange-500';
                        trigger.style.borderColor = 'var(--detail-accent-500)';
                        trigger.style.background = 'var(--detail-accent-500)';
                        trigger.style.color = '#ffffff';
                        if (badge) badge.className = 'rounded-full bg-white/25 px-2 py-0.5 text-xs uppercase tracking-wide';
                      } else {
                        trigger.className = 'inline-flex items-center gap-3 rounded-full border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm transition focus:outline-none focus:ring-2 focus:ring-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:focus:ring-slate-500';
                        trigger.style.borderColor = '';
                        trigger.style.background = '';
                        trigger.style.color = '';
                        if (badge) badge.className = 'rounded-full bg-gray-200 px-2 py-0.5 text-xs uppercase tracking-wide text-gray-700 dark:bg-slate-700 dark:text-slate-200';
                      }
                    }
