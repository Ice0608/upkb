...
<head>
    ...
    <title>SESOC - Info Institusi</title>
    ...
    <style>
        ...
        html.dark .ii-card { background: #1e293b; border-color: rgba(255,255,255,0.07); box-shadow: 0 4px 28px rgba(0,0,0,0.32); }
        ...
        html.dark .ii-search-input { background: #0f172a; border-color: rgba(255,255,255,0.1); color: #f1f5f9; }
        html.dark .ii-search-input::placeholder { color: #475569; }
        html.dark .ii-search-input:focus { background: #1e293b; }
        html.dark .ii-kc { background: #1e293b; border-color: rgba(255,255,255,0.07); }
        html.dark .ii-kc:hover { background: #243048; border-color: var(--institusi-info-500, #f97316); box-shadow: 0 8px 32px rgba(0,0,0,0.38); }
        html.dark .ii-kc__name { color: #f1f5f9; }
        html.dark .ii-kc__chip { background: rgba(255,255,255,0.06); border-color: rgba(255,255,255,0.09); color: #94a3b8; }
        html.dark .ii-kc__divider { background: rgba(255,255,255,0.07); }
        html.dark .ii-kc__toggle { border-color: rgba(255,255,255,0.12); color: #64748b; }
        html.dark .ii-kc__toggle:hover { background: rgba(255,255,255,0.06); border-color: var(--institusi-info-500, #d4af37); color: var(--institusi-info-500, #d4af37); }
        html.dark .ii-kc__penerangan { color: #94a3b8; }
        html.dark .ii-kc__tarikh { color: #475569; }
        html.dark .ii-empty { background: rgba(255,255,255,0.04); border: 1px dashed rgba(255,255,255,0.1); }
        html.dark .ii-no-results { color: #475569; }

        .ii-hero { position: relative; height: 420px; overflow: hidden; }
        .ii-hero__img { position: absolute; inset: 0; width: 100%; height: 140%; object-fit: cover; object-position: center top; will-change: transform; }
        .ii-hero__overlay { position: absolute; inset: 0; background: linear-gradient(to bottom, rgba(0,0,0,0.12) 0%, rgba(0,0,0,0.52) 55%, rgba(0,0,0,0.82) 100%); }
        .ii-hero__content { position: absolute; bottom: 0; left: 0; right: 0; padding: 2.5rem 2rem 2rem; max-width: 72rem; margin: 0 auto; }
        .ii-hero__badge { display: inline-flex; align-items: center; padding: 0.22rem 0.9rem; border-radius: 9999px; font-size: 0.68rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.09em; margin-bottom: 0.65rem; }
        .ii-hero__title { font-size: clamp(1.75rem, 4vw, 2.9rem); font-weight: 900; color: #fff; line-height: 1.15; margin-bottom: 0.45rem; text-shadow: 0 2px 14px rgba(0,0,0,0.55); }
        .ii-hero__addr { color: rgba(255,255,255,0.78); font-size: 0.9rem; margin-bottom: 1.2rem; }
        ...
        .ii-back-btn { position: absolute; top: 1.25rem; left: 1.5rem; display: inline-flex; align-items: center; gap: 0.5rem; background: rgba(0,0,0,0.32); backdrop-filter: blur(10px); color: #fff; font-size: 0.8rem; font-weight: 600; padding: 0.4rem 1.1rem; border-radius: 9999px; transition: background 0.2s; text-decoration: none; border: 1px solid rgba(255,255,255,0.18); }
        .ii-back-btn:hover { background: rgba(0,0,0,0.55); }

        .ii-card { background: #fff; border-radius: 1.75rem; padding: 2rem 2.25rem; box-shadow: 0 4px 28px rgba(0,0,0,0.06); border: 1px solid #f1f5f9; display: flex; gap: 1.5rem; align-items: flex-start; }
        .ii-card__icon-wrap { flex-shrink: 0; width: 3.5rem; height: 3.5rem; display: flex; align-items: center; justify-content: center; background: var(--institusi-info-100, #fff7e6); border-radius: 1rem; font-size: 1.4rem; }
        .ii-section-title { font-size: 1.3rem; font-weight: 800; color: #1e293b; }
        .ii-badge { font-size: 0.72rem; padding: 0.22rem 0.75rem; border-radius: 9999px; font-weight: 700; vertical-align: middle; }
        @media (max-width: 640px) {
            .ii-card--about { flex-direction: column; gap: 1rem; }
            .ii-card--about .ii-card__content { width: 100%; }
        }

        .ii-search-input { border: 1.5px solid #e2e8f0; border-radius: 9999px; padding: 0.5rem 1.2rem; font-size: 0.875rem; outline: none; width: 220px; background: #f8fafc; transition: border-color 0.2s, box-shadow 0.2s; }
        .ii-search-input:focus { border-color: var(--institusi-info-500, #f97316); box-shadow: 0 0 0 3px rgba(212,175,55,0.14); background: #fff; }

        .ii-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 3rem; background: #f8fafc; border-radius: 1.25rem; color: #94a3b8; text-align: center; }

        .ii-kc { border: 1.5px solid #f1f5f9; border-radius: 1.25rem; background: #fff; transition: box-shadow 0.28s, border-color 0.28s, transform 0.28s; overflow: hidden; }
        .ii-kc:hover { box-shadow: 0 8px 32px rgba(0,0,0,0.10); border-color: var(--institusi-info-500, #f97316); transform: translateY(-2px); }
        .ii-kc__header { display: flex; align-items: flex-start; gap: 1rem; padding: 1.15rem 1.35rem; }
        .ii-kc__actions { display: flex; align-items: center; gap: 0.6rem; margin-top: 0.7rem; flex-wrap: wrap; }
        .ii-kc__toggle { display: inline-flex; align-items: center; gap: 0.35rem; background: none; border: 1.5px solid #e2e8f0; border-radius: 9999px; padding: 0.3rem 0.85rem; font-size: 0.75rem; font-weight: 600; color: #64748b; cursor: pointer; transition: border-color 0.2s, color 0.2s, background 0.2s; }
        .ii-kc__toggle:hover { border-color: var(--institusi-info-500, #d4af37); color: var(--institusi-info-600, #b88912); background: var(--institusi-info-100, #fef3c7); }
        .ii-kc__icon { flex-shrink: 0; width: 2.75rem; height: 2.75rem; display: flex; align-items: center; justify-content: center; border-radius: 0.875rem; font-size: 1.1rem; background: var(--institusi-info-100, #fef3c7); color: var(--institusi-info-600, #b88912); transition: background 0.2s; }
        .ii-kc:hover .ii-kc__icon { background: var(--institusi-info-500, #d4af37); color: #fff; }
        .ii-kc__meta { flex: 1; min-width: 0; }
        .ii-kc__name { font-size: 1rem; font-weight: 700; color: #1e293b; line-height: 1.35; margin-bottom: 0.45rem; }
        .ii-kc__chips { display: flex; flex-wrap: wrap; gap: 0.5rem; }
        .ii-kc__chip { font-size: 0.8rem; font-weight: 600; padding: 0.35rem 0.85rem; border-radius: 9999px; background: #f1f5f9; color: #475569; display: inline-flex; align-items: center; gap: 0.4rem; white-space: nowrap; }
        .ii-kc__chevron { flex-shrink: 0; color: #94a3b8; font-size: 0.75rem; margin-top: 0.1rem; transition: transform 0.25s; }
        .ii-kc--open .ii-kc__chevron { transform: rotate(180deg); }
        .ii-kc__body { display: none; padding: 0 1.35rem 1.2rem; border-top: 1px solid #f1f5f9; }
        .ii-kc--open .ii-kc__body { display: block; }
        .ii-kc__penerangan { font-size: 0.85rem; color: #475569; line-height: 1.7; margin-bottom: 0.85rem; margin-top: 0.9rem; }
        .ii-kc__tarikh { font-size: 0.72rem; color: #94a3b8; }
        .ii-kc__cta { display: flex; align-items: center; justify-content: center; gap: 0.75rem; width: 100%; padding: 1rem 1.5rem; border-radius: 1rem; font-size: 1.05rem; font-weight: 800; letter-spacing: 0.02em; background: var(--institusi-info-500, #d4af37); color: #fff; text-decoration: none; transition: filter 0.2s, transform 0.2s, box-shadow 0.2s; box-shadow: 0 4px 20px rgba(0,0,0,0.18); margin-top: 0.85rem; }
        .ii-kc__cta:hover { filter: brightness(1.1); transform: translateY(-2px); box-shadow: 0 8px 32px rgba(0,0,0,0.22); }

        .ii-gallery-item { position: relative; border-radius: 1.25rem; overflow: hidden; aspect-ratio: 4/3; cursor: pointer; border: none; padding: 0; background: #e5e7eb; display: block; width: 100%; box-shadow: 0 2px 14px rgba(0,0,0,0.08); transition: box-shadow 0.28s, transform 0.28s; }
        .ii-gallery-item:hover { box-shadow: 0 10px 36px rgba(0,0,0,0.18); transform: translateY(-4px); }
        .ii-gallery-img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.5s cubic-bezier(0.22, 1, 0.36, 1); display: block; }
        .ii-gallery-item:hover .ii-gallery-img { transform: scale(1.09); }
        .ii-gallery-overlay { position: absolute; inset: 0; background: linear-gradient(to top, rgba(0,0,0,0.75) 0%, rgba(0,0,0,0.1) 55%, transparent 100%); display: flex; flex-direction: column; align-items: center; justify-content: flex-end; padding: 1.1rem; opacity: 0; transition: opacity 0.3s; }
        .ii-gallery-item:hover .ii-gallery-overlay { opacity: 1; }

        .ii-lb { position: fixed; inset: 0; z-index: 9999; background: rgba(0,0,0,0.92); display: flex; flex-direction: column; align-items: center; justify-content: center; opacity: 0; pointer-events: none; transition: opacity 0.25s; }
        .ii-lb--open { opacity: 1; pointer-events: all; }
        .ii-lb__img { max-width: min(90vw, 920px); max-height: 78vh; object-fit: contain; border-radius: 0.75rem; box-shadow: 0 20px 70px rgba(0,0,0,0.55); transition: transform 0.3s; }
        .ii-lb__cap { color: rgba(255,255,255,0.7); font-size: 0.88rem; margin-top: 1rem; text-align: center; }
        .ii-lb__close { position: absolute; top: 1.25rem; right: 1.25rem; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.18); color: #fff; width: 2.6rem; height: 2.6rem; border-radius: 50%; font-size: 1.1rem; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background 0.2s; }
...
<body class="no-bg {{ $institusiInfoType === 'tvet' ? 'institusi-info-page--tvet' : '' }} {{ $institusiInfoType === 'diploma' ? 'institusi-info-page--diploma' : '' }} {{ $institusiInfoType === 'sains kesihatan' ? 'institusi-info-page--sains-kesihatan' : '' }} {{ !$isThemed ? 'bg-gray-100' : '' }} text-gray-800">
    ...
    <div class="ii-hero">
        ...
        <a href="{{ route('pelajar.institusi', ['pelajar' => $pelajar->id, 'jenis' => $institusi->jenis_institusi]) }}" class="ii-back-btn">
                ...
                <div class="grid sm:grid-cols-2 gap-3" id="ii-kursus-grid">
                    ...
                    <div class="ii-kc" id="ii-kc-{{ $kursus->id }}" data-name="{{ strtolower($kursus->nama_kursus) }} {{ strtolower($kursus->kod_kursus) }}">
                        ...
                        <div class="px-1 pb-1.5">
                            <a href="{{ route('pelajar.infokursus', ['pelajar' => $pelajar->id, 'kursus' => $kursus->id]) }}" class="ii-kc__cta">
        ...
        <div class="ii-card ii-reveal" style="transition-delay:0.2s">
            <div class="w-full min-w-0">
                ...
                <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-5">
                    ...
                    <button class="ii-gallery-item" onclick="iiLightbox('{{ public_media_url($foto->imej ?? null) }}', '{{ addslashes($foto->penerangan ?? ('Fasiliti ' . $institusiEntityLabelLower)) }}')">
                        <img src="{{ public_media_url($foto->imej ?? null) }}" alt="Fasiliti" class="ii-gallery-img" loading="lazy">
