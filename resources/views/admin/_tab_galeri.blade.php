<div class="space-y-6">
    ...
    <div class="rounded-[2rem] border border-orange-100 bg-gradient-to-br from-orange-50 via-white to-amber-50 p-6 shadow-sm">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h3 class="text-xl font-semibold text-gray-800">Muat Naik Media</h3>
                <p class="mt-1 text-sm text-gray-500">Seret fail ke ruangan ini atau klik untuk pilih gambar dan video kursus.</p>
            </div>
        ...
        <form id="galeriUploadForm" action="{{ route('admin.storegaleri') }}" method="POST" enctype="multipart/form-data" class="mt-6 space-y-5">
            ...
            <label id="galeriDropzone" class="flex min-h-[260px] w-full cursor-pointer flex-col items-center justify-center rounded-[1.75rem] border-2 border-dashed border-orange-300 bg-white px-6 py-10 text-center shadow-sm transition hover:border-orange-400 hover:bg-orange-50">
                <div class="flex h-16 w-16 items-center justify-center rounded-full bg-orange-100 text-orange-600 shadow-sm">
            ...
            <div class="flex flex-col gap-3 sm:flex-row">
                ...
                <button type="reset" class="inline-flex flex-1 items-center justify-center rounded-full bg-white px-6 py-3 font-semibold text-gray-700 ring-1 ring-gray-200 shadow-sm transition hover:bg-gray-50">
    ...
    <div class="rounded-[2rem] border border-gray-200 bg-white p-6 shadow-sm">
        ...
        @if($galleries && $galleries->count() > 0)
            <div class="mt-6 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
                @foreach($galleries as $gallery)
                    ...
                    <div class="group overflow-hidden rounded-[1.75rem] border border-gray-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                        <div class="relative">
                            @if($isVideo)
        ...
        @else
            <div class="mt-6 rounded-[1.75rem] border border-dashed border-gray-300 bg-gray-50 px-6 py-14 text-center">
                <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-white text-gray-300 shadow-sm">
    ...
    <div id="galeriPreviewModal" class="fixed inset-0 z-50 hidden bg-black/80 px-4 py-8">
        <div class="mx-auto flex h-full w-full max-w-5xl items-center justify-center">
            <div class="relative w-full rounded-[1.75rem] bg-white p-4 shadow-2xl">
